...
from pep_compass.optimization.components.filters.ranked.scoring.helpers.mutation_potentials import DEFAULT_ALPHABET, LamsAnchorSimilarityPotential, ProjectedDirectionPairwiseSimilarityPotential, compose_mutant_distribution


class LamsScore(MutationPoolScore):
    """Score combinations by their worst compatible mutation pair."""
    ...
    def score_group(self, parent, mutations, tangent_space):
        ...
        distribution = compose_mutant_distribution(
            parent, bounded, potential, alphabet=self.alphabet,
            include_parent_residue=True, maximum_candidates=self.maximum_candidates,
        )
