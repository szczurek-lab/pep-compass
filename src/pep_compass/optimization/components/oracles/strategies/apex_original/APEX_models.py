...
class AMP_model(nn.Module):
    ...
    def forward_for_probs(self, x):

        # Extract embedding weights (num_embeddings, emb_dim) 
        # it is 23,566
        embedding_matrix = self.peptideEmb.aa_embedding.weight
        
        x = torch.einsum("bsv,vh->bsh", x, embedding_matrix)
